...
/* 设置缩放图片目录 */
define('IMAGECROPDIR', ROOT . '/../temp/%1$sx%2$s_mode%3$s/%4$s/%5$s'); // %1$s 宽, %2$s 高, %3$s 模式, , %4$s 目录, %5$s 文件
/* 默认缩放模式
...
/* 获取宽高 */
list($width, $height, $mode) = width_height_mode();
...
$old = ROOT . '/../' . $path;
/* 指定规格文件 */
$new = sprintf(IMAGECROPDIR, $width, $height, $mode, dirname($path), basename($path));
/* 存在源文件 */
if (file_exists($old))
{
    /* 不存指定规格文件 */
 ...
 * 获取宽高和缩放模式
 */
function width_height_mode()
{
    if ($query_string = from($_SERVER, 'QUERY_STRING'))
    {
        if (preg_match('/^([0-9]+)x([0-9]+)(?:m([1-5]))?$/', $query_string, $match))
        {
            if (from($match, 1) && from($match, 2))
            {
                return array($match[1], $match[2], from($match, 3, DEFAULT_MODE));
            }
        }
    }
    return show_404();
